...
export class SocketGateway {
  ...
  async handleConnection(socket: Socket) {
    ...
    if (!parsed?.refreshToken) return socket.disconnect();
  ...
  async getUsersNonViewingChat(
    userId: string,
    chatId: string,
  ): Promise<Array<{ userId: string; isViewing: boolean }>> {
    try {
      ...
      const viewingUserIds = responses
        .filter(
          (res: { userId: string; isViewing: boolean }) =>
            res.userId !== userId,
        )
        .map((res: { userId: string; isViewing: boolean }) => ({
          userId: res.userId,
          isViewing: res.isViewing,
        }));

      return [...new Set(viewingUserIds)] as unknown as Array<{
        userId: string;
        isViewing: boolean;
      }>; // those who are  online in the chat and not the sender
    } catch (err) {
      // timeout — treat as "nobody confirmed viewing"
      return [...new Set()] as unknown as Array<{
        userId: string;
        isViewing: boolean;
      }>;
    }
