...
export class AuthService {
  ...
  > {
    ...
    const template = await renderFile(
      join(process.cwd(), 'public/templates/Email', 'otp.ejs'),
      { name: user.name, otp },
    );
  ...
  async verifyRegistration(sessionDto: SessionDto): Promise<
    ApiResponse<{
      user: Partial<User>;
    }>
  > {
    const sessionExists = await this.sessionRepo.findBySessionId(
      sessionDto.sessionId,
    );

    if (!sessionExists) throw new HttpException('Session not found', 404);
  ...
  async login(signinDto: SingnInDto): Promise<
    ApiResponse<{
  ...
  async loginVerify(loginDto: SessionDto): Promise<
    ApiResponse<{
      sessionId: string;
      user: Partial<User>;
    }>
  > {
    ...
    ); // 15 min with access token logged in
    return {
      success: true,
      statusCode: 200,
  ...
  async authenticated(
    user: UserType,
  ): Promise<ApiResponse<{ user: UserType; authenticated: boolean }>> {
    ...
    await this.redis.set(
      ...
      'EX',
      this.redisExpire + 900, // 30 minutes
    );
    return {
      success: true,
      statusCode: 200,
