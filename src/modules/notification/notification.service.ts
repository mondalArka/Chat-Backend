...
export class NotificationService {
    ...
    async createNotification(body: NotificationDto) {
        return this.notificationRepo.createNotification(body as unknown as NotificationCreate);
    }

    async insertNotification(body: NotificationDto[]) {
        return this.notificationRepo.insertNotification(body as unknown as NotificationCreate[]);
    }

    async getPaginatedNotification(user: UserType, limit = 10, page = 1): Promise<ApiResponse<UserNotification[]>> {
        const { list = [], count = 0, page: listPage = 1, limit: listLimit = 10 } = await this.notificationRepo.getPaginatedNotification(user?.id, limit, page);
        return {
            success: true,
            statusCode: 200,
    ...
    async markAllRead(user: UserType): Promise<ApiResponse<{}>> {
        await this.notificationRepo.update({ user: { id: user?.id } }, { isRead: true });
        return {
            success: true,
            statusCode: 200,
    ...
    async readByChatId(user: UserType, chatId: string): Promise<ApiResponse<{}>> {
        await this.notificationRepo.update({ user: { id: user?.id }, chat: { id: chatId } }, { isRead: true });
        return {
            success: true,
            statusCode: 200,
            message: "Notifications read successfully"
