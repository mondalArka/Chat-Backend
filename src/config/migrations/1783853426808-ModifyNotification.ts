...
export class ModifyNotification1783853426808 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`notifications\` ADD \`userId\` bigint UNSIGNED NOT NULL`,
    );

    // 4. Composite index — userId first, as requested
    await queryRunner.query(
      `CREATE INDEX \`chat_user_notifications_idx\` ON \`notifications\` (\`userId\`, \`chatId\`)`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`notifications\` DROP FOREIGN KEY \`FK_user_notifications\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`notifications\` DROP FOREIGN KEY \`FK_chat_notifications\``,
    );
    ...
    await queryRunner.query(
      `DROP INDEX \`chat_user_notifications_idx\` ON \`notifications\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`notifications\` DROP COLUMN \`userId\``,
    );
    await queryRunner.query(
      `CREATE INDEX \`chat_notifications_idx\` ON \`notifications\` (\`chatId\`)`,
    );
    await queryRunner.query(`
            ALTER TABLE \`notifications\`
            ADD CONSTRAINT \`FK_chat_notifications\`
